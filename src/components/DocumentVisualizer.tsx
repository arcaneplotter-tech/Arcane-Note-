...
export default function DocumentVisualizer({ data, theme = MapTheme.DEFAULT, layout = MapLayout.TREE_HORIZONTAL, onReorder }: Props) {
  ...
  const getThemeStyles = (t: string) => {
    ...
    switch (t) {
      ...
      case MapTheme.VIBRANT:
        return {
          bg: '#ffffff', line: '#f0f0f0', text: '#000000', font: 'Inter, sans-serif', shape: 'rect',
          getFill: (d: number) => depthColor(d, ['#FF00FF', '#00FFFF', '#FFFF00', '#FF0000']),
          getStroke: () => '#000000',
          strokeWidth: 3, shadow: 'drop-shadow(5px 5px 0px rgba(0,0,0,1))', connectionType: 'straight'
        };
      case MapTheme.MINECRAFT:
        return {
          bg: '#c6c6c6', line: '#373737', text: '#373737', font: '"VT323", monospace', shape: 'rect',
          getFill: (d: number) => depthColor(d, ['#4caf50', '#795548', '#1a1a1a', '#c6c6c6']),
          getStroke: () => '#373737',
          strokeWidth: 4, shadow: 'drop-shadow(4px 4px 0px rgba(0,0,0,0.5))', connectionType: 'step'
        };
      default: 
        return {
